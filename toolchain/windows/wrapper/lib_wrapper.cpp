...
std::wstring FindVisualStudioPath() {
    ...
    hr = CoCreateInstance(CLSID_SetupConfiguration, NULL, CLSCTX_INPROC_SERVER, IID_ISetupConfiguration, (LPVOID*)&pConfig);
    if (FAILED(hr)) { 
        CoUninitialize(); 
...
int main(int argc, char* argv[]) {
    ...
    if (outPath.has_parent_path()) {
        try { fs::create_directories(outPath.parent_path()); } catch(...) {}
    }
